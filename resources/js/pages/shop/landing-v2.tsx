                ...
                <section className="border-b border-gray-100 bg-linear-to-b from-primary/5 to-white">
                    <div className="mx-auto max-w-5xl px-4 py-10 text-center md:py-16">
                        ...
                        {landingPage.hero_text && (
                            <p className="mx-auto mb-6 max-w-2xl whitespace-pre-line text-lg leading-relaxed text-gray-500 md:text-xl">{landingPage.hero_text}</p>
                        )}
                    ...
                    return (
                        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4" onClick={() => setVideoOpen(false)}>
                            <div className="relative w-full max-w-3xl" onClick={(e) => e.stopPropagation()}>
                                <button onClick={() => setVideoOpen(false)} className="absolute -top-10 right-0 flex items-center gap-1 text-white/80 hover:text-white">
                                    <X className="h-6 w-6" />
                                </button>
                                <div className="aspect-video w-full overflow-hidden rounded-xl">
                                    <iframe src={`${embedUrl}?autoplay=1`} className="h-full w-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen title={landingPage.title} />
                                </div>
                            </div>
                        </div>
                    );
                                        ...
                                        <ul className="space-y-3">
                                            {landingPage.features.map((f, i) => (
                                                <li key={i} className="flex items-start gap-3">
                                                    ...
                                                    <div>
                                                        <p className="break-words text-sm font-semibold text-gray-800">{f.title}</p>
                                                        {f.desc && <p className="whitespace-pre-line break-words text-xs text-gray-500">{f.desc}</p>}
                                                    </div>
                                                </li>
                                            ))}
                                        </ul>
                            ...
                            <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-4">
                                {landingPage.why_buy.map((item, i) => (
                                    <div key={i} className="flex flex-col items-center rounded-xl border border-gray-200 bg-white p-5 text-center shadow-sm">
                                        ...
                                        <h4 className="mb-1 break-words text-sm font-bold text-gray-800">{item.title}</h4>
