...
export default function EditOrder({ order, products, statuses, emailEnabled }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        ...
        shipping: String(parseFloat(order.shipping ?? '0').toFixed(2)),
        items: order.items.map((item) => ({
            product_id: String(item.product_id),
            variant_id: item.product_variant_id ? String(item.product_variant_id) : '',
            quantity: String(item.quantity),
            custom_price: String(parseFloat(item.price).toFixed(2)),
        })) as OrderItemRow[],
    });

    function addItem() {
        setData('items', [...data.items, { product_id: '', variant_id: '', quantity: '1', custom_price: '' }]);
    }
    ...
    const getDefaultPrice = (item: OrderItemRow): number => {
    ...
    const getEffectivePrice = (item: OrderItemRow): number => {
        const custom = parseFloat(item.custom_price);
        if (!isNaN(custom) && item.custom_price !== '') return custom;
        return getDefaultPrice(item);
    };

    const subtotal = data.items.reduce((sum, item) => {
        return sum + getEffectivePrice(item) * (parseInt(item.quantity) || 0);
    }, 0);

    const shippingNum = parseFloat(data.shipping) || 0;
                            ...
                            <div className="rounded-xl border border-sidebar-border/70 p-4 dark:border-sidebar-border">
                                ...
                                <div className="space-y-4">
                                    {data.items.map((item, index) => {
                                        ...
                                        const lineTotal = effectivePrice * (parseInt(item.quantity) || 0);
                                            ...
                                            <div key={index} className="flex flex-wrap items-start gap-3 rounded-lg border border-input p-3">
                                                ...
                                                <div className="w-24">
                                                    <label className="mb-1 block text-xs font-medium text-muted-foreground">
                                                        Price
                                                        {defaultPrice > 0 && (
                                                            <span className="ml-1 text-muted-foreground/60">(default: ৳{defaultPrice.toFixed(0)})</span>
                                                        )}
                                                    </label>
                                                    <input
                                                        type="number"
                                                        ...
                                                        placeholder={defaultPrice > 0 ? String(defaultPrice.toFixed(2)) : '0.00'}
                                                        className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                                                    />
                                                </div>
                            ...
                            <div className="rounded-xl border border-sidebar-border/70 p-4 dark:border-sidebar-border">
                                ...
                                <div className="space-y-3 text-sm">
                                    ...
                                    <div className="flex items-center justify-between gap-2">
                                        <label htmlFor="shipping" className="shrink-0 text-muted-foreground">Shipping</label>
                                        <input
                                            id="shipping"
                                            type="number"
                                            min="0"
